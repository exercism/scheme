  ...
  (let ((exercisms (lookup 'exercises track-config)))
    (cond ((find (lambda (exercism)
          ...
          (else (error 'check-config-for
                       "please add problem to config/config.sexp"
                       problem)))))
  ...
  (let ((dir (format "_build/exercises/~a" problem))
    ...
    (with-output-to-string
      (lambda ()
        (let ((skeleton
               (begin
                 (load (format "~a/~a" dir (lookup 'skeleton implementation)))
                 (test)))
              (example
               (begin
                 (load (format "~a/~a" dir (lookup 'solution implementation)))
                 (test))))
          (unless (and (eq? 'failure skeleton) (eq? 'success example))
