...
(define (test . args)
  (apply
    ...
    (list
      (lambda ()
        (test-success "year not divisible by 4 in common year" eq?
          leap-year? '(2015) #f))
      (lambda ()
        (test-success
          "year divisible by 2, not divisible by 4 in common year" eq?
          leap-year? '(1970) #f))
      (lambda ()
        (test-success
          "year divisible by 4, not divisible by 100 in leap year" eq?
          leap-year? '(1996) #t))
      (lambda ()
        (test-success
          "year divisible by 4 and 5 is still a leap year" eq?
          leap-year? '(1960) #t))
      (lambda ()
        (test-success
      ...
      (lambda ()
        (test-success
          ...
          eq? leap-year? '(1900) #f))
      (lambda ()
        (test-success "year divisible by 400 in leap year" eq?
          leap-year? '(2000) #t))
      (lambda ()
        (test-success
